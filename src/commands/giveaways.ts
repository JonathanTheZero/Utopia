...
export async function startGiveaway(message: Message, args: string[], client: Client) {
    ...
    if (args.length < 4 || parseInt(args[3].replace(/[,]/g, '')) < 1 || parseInt(args[0].replace(/[,]/g, '')) < 1) return message.reply("please follow the syntax `.start-giveaway <amount> <currency> <winners> <ending>`");
