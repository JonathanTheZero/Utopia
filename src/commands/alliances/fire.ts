...
import { getAlliance, editAllianceArray, updateValueForUser } from "../../utils/databasehandler";
...
export async function fire(name: string, author: user, member: user): Promise<string> {
    ...
    if (member.allianceRank == "M") {
        editAllianceArray(name, "members", "$pull", member._id);
    }
    else {
        editAllianceArray(name, "coLeaders", "$pull", member._id);
    }
