    ...
    for i in range(len(field_list)):
        for j in range(len(field_list[i])):
            if isinstance(field_list[i][j], str) :
                ...
                if a[0] == "1":
                    if a[1] == "0":
                        hexa = Image.open(BytesIO(requests.get(config.HEX_RED1).content))
                    if a[1] == "1":
                        hexa = Image.open(BytesIO(requests.get(config.HEX_RED2).content))
                    if a[1] == "2":
                        hexa = Image.open(BytesIO(requests.get(config.HEX_RED3).content))
                elif a[0] == "2":
                    if a[1] == "0":
                        hexa = Image.open(BytesIO(requests.get(config.HEX_BLUE1).content))
                    if a[1] == "1":
                        hexa = Image.open(BytesIO(requests.get(config.HEX_BLUE2).content))
                    if a[1] == "2":
                        hexa = Image.open(BytesIO(requests.get(config.HEX_BLUE3).content))
