...
export async function loan(message: Message, args: string[], user: user){
    ...
    if(!user.loan){
      ...
      if (userloan > maxloan)
        ...
        updateValueForUser(message.author.id, "money", userloan);
...
export async function payback(message: Message, args: string[], user: user){
    ...
    if(user.loan) {
      ...
      updateValueForUser(message.author.id, "money", -1*userpayment);
      if(user.loan <= userpayment) {
        ...
        updateValueForUser(message.author.id, "money", diff);
        return;
      }
